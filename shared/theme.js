// AUTO-COPIED FROM 4veco-platform/engines/ — DO NOT EDIT HERE
// Shared domain colors per chapter — loaded once, used by all game UIs.
// Each game's UI falls back to this map when the data file has no domainColors.
var DOMAIN_COLORS = {
  "3.1": { primary: "#17A2B8", primaryDk: "#117A8B", primaryLt: "#E8F8FB", accent: "#F8C471", navy: "#1E2761" },
  "3.2": { primary: "#1A5276", primaryDk: "#154360", primaryLt: "#EBF5FB", accent: "#F8C471", navy: "#1E2761" },
  "3.3": { primary: "#E67E22", primaryDk: "#BA6A1C", primaryLt: "#FEF5E7", accent: "#F8C471", navy: "#1E2761" },
  "3.4": { primary: "#1E8449", primaryDk: "#186A3B", primaryLt: "#E8F8F0", accent: "#F8C471", navy: "#1E2761" },
  "3.5": { primary: "#7D3C98", primaryDk: "#6C3483", primaryLt: "#F4ECF7", accent: "#F8C471", navy: "#1E2761" },
  "1.1": { primary: "#17A2B8", primaryDk: "#117A8B", primaryLt: "#E8F8FB", accent: "#F8C471", navy: "#1E2761" }
};
